...
export const adapterHH: Adapter<"hh"> = {
  adaptParams(params: VacancyParams): HHParams {
    ...
    return {
      ...
      salary:
        Math.floor(
          ((params.filters.salary?.from ?? 0) +
            (params.filters.salary?.to ?? 0)) /
            2,
        ) || null,
      only_with_salary:
        params.filters?.salary?.from || params.filters?.salary?.to
          ? true
          : false,
      period:
        (params.filters?.period === 0 ? 99 : params.filters?.period) ?? 99,
      schedule: params.filters?.type ? typeMap[params.filters?.type] : null,
      area: params.filters?.place
        ? `place~${parsedPlace.id ? `id~~${parsedPlace.id}` : `name~~${parsedPlace.name}`}`
        : null,
    };
  },
  ...
  adaptVacancies(vacancies: Array<VacancyHH>): Array<Vacancy> {
    return vacancies.map((vacancy) => {
      return {
        ...
        description: combineDutyAndReqToDesc(
