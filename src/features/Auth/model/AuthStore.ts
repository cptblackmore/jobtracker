...
export class AuthStore {
  ...
  get resendCooldown() {
    ...
    const seconds = Math.floor(
      (Date.parse(this.user.nextResendAt) - this.currentTime) / 1000,
    );
  ...
  async login(email: string, password: string) {
    ...
    try {
      ...
      authChannel.postMessage({ type: "login", payload: toJS(this.user) });
  ...
  async logout(reason?: string, severity?: AlertType["severity"]) {
    try {
      ...
      if (reason)
        this.alertsStore.addAlert(createAlert(reason, severity || "error"));
    } catch (e) {
      if (e instanceof Error) {
        this.alertsStore.addAlert(createAlert(e.message, "error"));
      }
  ...
  async resend() {
    ...
    try {
      ...
      this.alertsStore.addAlert(
        createAlert(
          `На вашу почту ${this.user.email} отправлено письмо для подтверждения.`,
          "info",
        ),
      );
    } catch (e) {
  ...
  async checkAuth() {
    ...
    try {
      ...
      await broadcastRequestWithFallback(
        ...
        () => electLeader(this, authChannel, () => this.refresh()),
        500,
      );
    } catch (e) {
      if (e instanceof Error) {
