  ...
  const handleDismiss = () => {
    if (alertsStore.currentAlert?.dismissedKey) {
      const newDismissedAlerts = new Set(
        [...JSON.parse(window.localStorage.getItem('dismissedAlerts') || '[]'), alertsStore.currentAlert.dismissedKey]
      );
      window.localStorage.setItem('dismissedAlerts', JSON.stringify(Array.from(newDismissedAlerts)));
    }
