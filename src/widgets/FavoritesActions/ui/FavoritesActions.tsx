...
export const FavoritesActions: React.FC<Props> = observer(
  ({ clearDisplayedFavorites, resetDisplayedFavorites }) => {
    const {
      modalOpen,
      setModalOpen,
      progress,
    ...
    return (
      <>
        ...
        <Paper sx={{ p: isSmUp ? 2 : 1, borderRadius: 2, boxShadow: 2, mb: 2 }}>
          {isSmUp ? (
            <Box display="flex" justifyContent="space-between" gap={2}>
              ...
              {buttons.map((button, i) => {
                ...
                const disabled =
                  isActive ||
                  (button.disableOnLoading && isLoading) ||
                  (button.disableOnEmpty && favoritesStore.ids.length === 0);
                ...
                return (
                  <Button
                    ...
                    disabled={disabled}
                    endIcon={
                      isActive && <CircularProgress size={15} color="inherit" />
                    }
