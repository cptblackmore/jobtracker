...
export const useSourcesStatus = () => {
  ...
  const [statuses, setStatuses] = useState<Record<Sources, 'success' | 'error' | 'pending'>>(initializeStatuses());
  ...
  useEffect(() => {
    ...
    async function fetchStatuses() {
      const entries: [Sources, 'success' | 'error' | 'pending'][] = await Promise.all(
        typedKeys(sourcesRegistry).map(async (source) => {
          try {
            const result = await getOnlineStatuses(sourcesIdsMapping[source], signal);
      ...
      setStatuses(Object.fromEntries(entries) as Record<Sources, 'success' | 'error' | 'pending'>);
    }
