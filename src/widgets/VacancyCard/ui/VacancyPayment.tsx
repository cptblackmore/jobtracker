...
export const VacancyPayment: React.FC<Props> = ({
  vacancy,
  variant = "column",
}) => {
  const paymentFrom = vacancy.paymentFrom
    ? formatNumberByCurrency(vacancy.paymentFrom, vacancy.currency)
    : null;
  const paymentTo = vacancy.paymentTo
    ? formatNumberByCurrency(vacancy.paymentTo, vacancy.currency)
    : null;
    ...
    <>
      ...
      >
        {!paymentFrom && !paymentTo && (
          <T textAlign="end" sx={textStyle}>
            Зарплата не указана
          </T>
        )}
        {paymentFrom ? (
          <T whiteSpace="nowrap" textAlign="end" sx={textStyle}>
            от <b>{paymentFrom}</b>
          </T>
        ) : (
          ""
