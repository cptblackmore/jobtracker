...
export const usePlaceFilter = () => {
  ...
  const debouncedPlacesRequest = useMemo(() => 
    ...
    try {
      const foundPlaces = await PlacesService.getPlaces(newPlace, new AbortController().signal);
      if (foundPlaces.data.length) {
        setSuggestedPlaces(foundPlaces.data);
      } else {
        setSuggestedPlaces([{id: '', name: 'Мест не найдено', type: 'city', isMeta: true}]);
      }
    } catch {
      setSuggestedPlaces([{id: '', name: 'Мест не найдено', type: 'city', isMeta: true}]);
    }
  ...
  const handlePlaceInputChange = useCallback((e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const handlePlaceChange = useCallback((e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const newPlace = e.target.value as string | Place;
    if (typeof newPlace === 'string') {
      const matched = suggestedPlaces.find(place => place.name.toLowerCase() === newPlace.toLowerCase());
      if (matched) {
  ...
  return { place, resetPlace, setPlace, handlePlaceInputChange, handlePlaceChange, suggestedPlaces, formattedPlace, setFormattedPlace };
};
