...
export const SalaryFilter: React.FC<Props> = ({
  handleSalaryChange,
  salaryFilter,
  handleInvalid,
}) => {
  ...
  return (
    <Box
      component="fieldset"
      sx={{ opacity: salaryFilter.enabled ? 1 : 0.5, border: "none" }}
    >
      <legend style={{ fontSize: 0 }}>{filterLabelsMap.salaryRange}</legend>
      <Box px={{ xs: 1.5, sm: 1 }}>
        <Slider
          value={[
            salaryFilter?.from ?? SALARY_MIN,
            salaryFilter?.to ?? SALARY_MAX,
          ]}
          onChange={(_, newValue) =>
            handleSalaryChange({
              action: "slider",
          ...
          marks={[
            ...
            {
              value: Math.floor((SALARY_MAX - SALARY_MIN) / 2),
              label: `${Math.floor((SALARY_MAX - SALARY_MIN) / 2 / 1000)}k`,
            },
          ...
          getAriaLabel={(index) =>
            index === 0 ? "Минимальная зарплата" : "Максимальная зарплата"
          }
          getAriaValueText={(value, index) =>
            index === 0 ? `${value} рублей` : `${value} рублей`
          }
        />
      </Box>
