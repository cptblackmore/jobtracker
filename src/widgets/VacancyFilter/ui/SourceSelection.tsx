...
import { alpha, Box, Checkbox, Fade, FormControl, FormControlLabel, FormGroup, IconButton, lighten, Tooltip, useMediaQuery, useTheme } from '@mui/material';
...
import { AriaInformer, ThemesContext, VisuallyHiddenTypography } from '@shared/ui';
    ...
    <FormControl component='fieldset' fullWidth >
      ...
      <AriaInformer>
        {sources
          ...
          .map((s) => `Источник ${sourcesRegistry[s.source].styles.name} отключён из-за несовместимости`)
          .join('. ')
        }
      </AriaInformer>
        ...
        {sources.map(source => (
          ...
          >
              ...
              label={
                <>
                  ...
                  <VisuallyHiddenTypography variant='p' >
                    {source.checked && !source.incompatible ? 'Активен' : 'Не активен'}
                    {source.incompatibleFilters ? 'Имеет особенности - изучите справку' : ''}
            ...
            {source.incompatibleFilters && (
              ...
              >
                <IconButton sx={{p: 0.5, ml: 0, my: 0.5, mr: 0.5}} size={isSmUp ? 'medium' : 'small'} >
