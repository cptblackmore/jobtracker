...
export const useVacancyList = (initialParams: VacancyParams) => {
  ...
  const [state, dispatch] = useReducer(vacancyListReducer, {
    params: {
      ...initialParams,
      filters: parseUrlSearch() || initialParams["filters"],
    },
    vacancies: [],
  });
  ...
  const fetchAndUpdateVacancies = async (
    actionType: ActionVacancies,
    signal: AbortSignal,
  ) => {
    setIsLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    if (!isEqual({ ...parsedFilters }, { ...state.params.filters })) {
      dispatch({
        type: "SET_FILTERS",
        filters: parsedFilters || initialParams["filters"],
      });
    }
  }, [location.search]);
  ...
  return {
    ...
    isLoading,
    alertsStore,
    toNextPage: useCallback(
      () => dispatch({ type: "SET_PAGE", page: state.params.page + 1 }),
      [dispatch, state.params.page],
    ),
